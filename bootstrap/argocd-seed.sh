...
#    그래서 --set도, 인라인 heredoc 매니페스트도 쓰지 않는다. 예외는 없다.
#    저장소가 public이라 ArgoCD는 repository Secret 없이 익명으로 읽는다.
#
# ⚠️ bash 3.2 호환으로 쓴다 — macOS 기본 bash가 3.2이고, 이 스크립트는 workbench 뿐 아니라
...
단계는 순서대로 실행되며 각 단계가 다음 단계의 전제다.
  ...
  ℹ️ Access Entry 단계는 self-managed에 없다 — ArgoCD가 클러스터 안에 있다.
     spoke 클러스터를 붙일 때만 필요하며 그것은 Terraform 소관이다.
     repository Secret 단계도 없다 — 저장소가 public이라 ArgoCD가 익명으로 읽는다.
...
선택 환경변수
  ...
  ARGOCD_CHART_VERSION  기본 10.9.1        (올릴 땐 argocd CLI도 같이)
...
# 저장소가 익명으로 읽히는지 — ArgoCD는 repository Secret 없이 root App의 repoURL을 읽으므로
# 저장소가 private으로 돌아가면 sync가 조용히 멈춘다(root App이 Unknown에 머문다). 그 실패를
# apply 전에 드러낸다. credential.helper를 비워 이 머신에 캐시된 자격증명이 검사를 통과시키지
# 못하게 한다.
GITOPS_REPO_URL=$(awk '/^ *repoURL:/{print $2; exit}' "$ROOTAPP_FILE")
[[ -n "$GITOPS_REPO_URL" ]] || die "root-app.yaml에서 repoURL을 읽지 못했다"
GIT_TERMINAL_PROMPT=0 git -c credential.helper= ls-remote --exit-code --heads "$GITOPS_REPO_URL" >/dev/null 2>&1 \
  || die "저장소를 익명으로 읽지 못한다(private으로 돌아갔는가): $GITOPS_REPO_URL. ArgoCD도 못 읽는다"
...
# ─────────────────────────────────────────────────────────────────────────────
# 2·3·4단계 — 커밋본을 그대로 apply. 각 단계가 다음의 전제다.
...
#    bash 3.2·5.x 모두 `set -e`가 && 리스트의 앞 명령 실패를 면제하므로 **조기 종료는 없다.**
#    문제는 다른 데 있다 — 그런 줄이 **마지막 문장이면 스크립트 종료 코드가 1**이 된다.
#    즉 `--to 3`으로 정상 실행한 seed가 호출자(CI·wrapper)에게 **실패로 보인다.**
#    if 블록은 건너뛰어도 0이다.
